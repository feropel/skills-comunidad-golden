...
2. Corre el inventario determinista:
   ...
   ```
   El script reporta: blindaje (chflags uchg / chmod 0444), árbol de archivos con líneas, frontmatter, referencias rotas (mencionadas pero inexistentes), archivos huérfanos (existen pero nadie los menciona), y chequeo de sintaxis de scripts (bash -n / ast.parse de Python, sin escribir nada en la skill auditada).
...
> 🔴 El script valida Python con `py_compile`, que escribe bytecode — `scripts/inventario.sh:97` — en una skill blindada la escritura falla y reporta un error de sintaxis FALSO. Arreglo: validar con `ast.parse` (solo lectura). *(Hallazgo real de la v1.0 de esta misma skill, ya corregido.)*
